        ...
        if payload.message_id == self.message_id and str(payload.emoji) == self.emoji:
            ...
            if role and member:
                try:
                    ...
                    await member.send("I'm unable to remove the role due to missing permissions.")
                except Exception as e:
    ...
    async def add_reaction(self, ctx, message_id: str, channel_id: str, emoji: str):
        if any(role.name in ["Admins", "Moderators", "Owner", "Staff", "Head Admin", "Head Moderator", "Trainee Moderators"] for role in ctx.author.roles):
            try:
                ...
                await ctx.respond(f"Reaction {emoji} added to message ID {message_id}.")
            except ValueError:
                await ctx.respond("Invalid message ID or channel ID format. Please enter valid integers.")
